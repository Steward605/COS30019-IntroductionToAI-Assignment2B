...
# KEEP ONLY NEEDED COLUMNS AND CLEAN THEM
# =======================================
...
# CONVERT WIDE DATA TO LONG TIME-SERIES DATA
# ==========================================
...
# AGGREGATE 15-MINUTE FLOW INTO HOURLY FLOW
# =========================================
...
# DROP SITES WITH TOO LITTLE DATA
# ===============================
...
# SCALE FLOW VALUES USING TRAINING DATA ONLY
...
def create_model_input_sequences(flow_split_df, input_history_steps, prediction_horizon):
    ...
    for _, site_flow_df in flow_split_df.groupby("scats_site_number"):
        ...
        hours = site_flow_df["timestamp"].dt.hour.to_numpy(dtype=np.float32)
        ...
        for start_index in range(max_start_index):
            ...
            flow_window = scaled_flow_values[start_index:end_index]
            hour_window = hours[start_index:end_index]

            sin_h = np.sin(2 * np.pi * hour_window / 24)
            cos_h = np.cos(2 * np.pi * hour_window / 24)

            sequence = np.stack([flow_window, sin_h, cos_h], axis=1)
