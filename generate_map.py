...
for _, row in edges_df.iterrows():
    ...
    folium.PolyLine(
        locations=[
            [start_node.iloc[0]["latitude"], start_node.iloc[0]["longitude"]],
            [end_node.iloc[0]["latitude"],   end_node.iloc[0]["longitude"]]
        ],
        color="#3b82f6",
        weight=2,
        opacity=0.5
...
if len(route_sites) >= 2:
    for i in range(len(route_sites) - 1):
        ...
        folium.PolyLine(
            ...
            color="#f59e0b",  # yellow/amber like Google Maps route color
            weight=6,
            opacity=0.9
        ).add_to(m)
