...
CONNECTED_NODES = get_config_value(CONFIG, ["visual", "connected_nodes"], "#3330ff")
...
class TBRGSApp(ctk.CTk):
    ...
    def draw_map(self):
        ...
        self.draw_connection_highlights(width, height)
    ...
    def draw_map_legend(self, width, height):
        ...
        #self.draw_legend_item(x + 14, y + 78, CONNECTED_NODES, "Connected nodes")
    ...
    def draw_connection_highlights(self, width, height):
        if self.selected_node_info is None:
            return
        selected_site = self.selected_node_info["site"]
        connected_nodes = self.selected_node_info["connected_nodes"]
        if selected_site not in self.node_lookup:
            return
        x1, y1 = self.latlon_to_canvas(
            self.node_lookup[selected_site]["latitude"],
            self.node_lookup[selected_site]["longitude"],
            width,
            height
        )
        for connected_site in connected_nodes:
            if connected_site not in self.node_lookup:
                continue
            x2, y2 = self.latlon_to_canvas(
                self.node_lookup[connected_site]["latitude"],
                self.node_lookup[connected_site]["longitude"],
                width,
                height
            )
            self.canvas.create_line(x1, y1, x2, y2, fill= CONNECTED_NODES,width=4)
